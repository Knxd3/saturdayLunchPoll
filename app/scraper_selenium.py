"""Selenium-powered scraper for TheFork listings.

This mirrors ``app.scraper`` but drives a real browser session so Cloudflare/
anti-bot protections are less likely to block us.
"""
...
def _create_driver() -> webdriver.Chrome:
    ...
    Looks for ``CHROMEDRIVER`` env var; otherwise relies on chromedriver in PATH.
    """
...
def browser(driver: webdriver.Chrome | None = None) -> Iterator[webdriver.Chrome]:
    """Context manager ensuring the driver quits if we create it here."""

    created = driver is None
    active_driver = driver or _create_driver()
    try:
        yield active_driver
    finally:
        if created:
...
def _fetch_page_source(driver: webdriver.Chrome, url: str) -> str:
    ...
    try:
        ...
        # If the primary container never shows, fall back to whatever HTML we have.
        pass
    return driver.page_source


def scrape_restaurants(base_url: str | None = None, pages: int = 3, driver: webdriver.Chrome | None = None):
    """Scrape listings using Selenium to fetch the HTML for each page."""
...
__all__ = ["scrape_restaurants", "_create_driver"]
